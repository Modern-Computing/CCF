...
for t in *_test; do
    llvm-profdata-7 merge -sparse "$t".profraw -o "$t".profdata
    llvm-cov-7 show -instr-profile "$t".profdata -object "$t" -ignore-filename-regex="(boost|openenclave|3rdparty|/test/)" > "$t"_coverage.txt
    bash <(curl -s https://codecov.io/bash) -t "${CODECOV_TOKEN}" -f "$t"_coverage.txt -F "$t"
done
